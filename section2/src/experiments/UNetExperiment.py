...
class UNetExperiment:
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if batch["image"].shape[0] < self.config.batch_size:
                batch_size = batch["image"].shape[0]
            
            image = batch["image"][:,0,:,:]
            data = image.reshape((batch_size, 1, self.config.patch_size, self.config.patch_size))  
            data = data.to(self.device, dtype=torch.float)

            seg = batch["seg"][:,0,:,:]
            target = seg.reshape((batch_size, 1, self.config.patch_size, self.config.patch_size))        
            ...
            # ANSWER: it represents segmentation values on different planes. the last for z, before it for y and before it for x.
...
#                 exit()
                ...
                loss = self.loss_function(prediction, target[:, 0, :, :])
                #loss.requires_grad=True#================== should remove?
...
#             values, count = np.unique(pred_label, return_counts=True)
#             print(pred_label.shape)
#             print(values)
#             print(count)
            
#             values, count = np.unique(x["seg"], return_counts=True)
#             print(x["seg"].shape)
#             print(values)
#             print(count)
            ...
            # Answer: Low Specificity -> segmenting more but incorrectly
            # Answer: Low Sensetivity -> segmenting less that required. Implemented as an example
